...
- (id)initWithFrame:(CGRect)frame
{
  if ((self = [super initWithFrame:frame])) {
    ...
    for (int i=0; i<6; i++) {
      for (int j=0; j<7; j++) {
          CGRect r = CGRectMake(j*(iPad ? kTileSize_iPad.width :kTileSize.width), i*(iPad ? kTileSize_iPad.height : kTileSize.height), (iPad ? kTileSize_iPad.width : kTileSize.width), (iPad ? kTileSize_iPad.height : kTileSize.height));
...
- (void)drawRect:(CGRect)rect
{
  CGContextRef ctx = UIGraphicsGetCurrentContext();
    CGContextDrawTiledImage(ctx, (CGRect){CGPointZero,(iPad ? kTileSize_iPad : kTileSize)}, [[UIImage imageNamed:@"Kal.bundle/kal_tile.png"] CGImage]);
}
...
- (void)sizeToFit
{
    self.height = 1.f + (iPad ? kTileSize_iPad.height : kTileSize.height) * numWeeks;
}
